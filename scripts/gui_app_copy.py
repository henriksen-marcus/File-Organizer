...
class App(customtkinter.CTk):
    def __init__(self):
        ...
        # Canvas for scrolling the entire window
        self.canvas = customtkinter.CTkCanvas(self, highlightthickness=0, bg="#333333")
        ...
        # Bind scrolling over the canvas to a function
        self.canvas.bind_all("<MouseWheel>", self.scroll) 

        # Pack the scrollbar and canvas
        scrollbar.pack(side="right", fill="y")
        self.canvas.pack(anchor="n", fill="both", expand=True, padx=0, pady=0, ipadx=0, ipady=0)

        # Template frame
        frame = customtkinter.CTkFrame(self.canvas, border_width=0)

        # Make a window in the canvas using the template, where everything will be
        window = self.canvas.create_window((0, 0), window=frame, anchor="n", )

        #self.canvas.itemconfigure(window, )

        # Main frame spanning the entire window, this goes in the canvas frame
        self.main = customtkinter.CTkFrame(frame, corner_radius=0, width=self.winfo_screenwidth() // 7, border_width=0)
        ...
        #for thing in range(10):
         #   customtkinter.CTkButton(self.main, text=f"Button  {thing}").grid(column=1,pady=10,padx=10)
